...
public class SoundPoolManager implements SoundPool.OnLoadCompleteListener {
    ...
    private static final Vector<JPSound> sounds = new Vector<>();
    ...
    public void init() {
        ...
        for (JPSound sound : sounds) {
            soundsIdMap.put(sound.getRome(), new JPSoundWithID(sound, null));
        }
    ...
    public void play(String rome) {
        ...
        JPSoundWithID jpSoundWithID = soundsIdMap.get(rome);
        ...
        if (jpSoundWithID.getId() != null) {
            soundPool.play(jpSoundWithID.getId(), 1, 1, 0, 0, 1);
        } else {
            int id = soundPool.load(App.getInstance(), jpSoundWithID.getSound().getResId(), 1);
            jpSoundWithID.setId(id);
        }
